...
$(document).ready(function () {
  ...
  var deleteFileModal = $('#deleteFileModal');
  var closeDeleteFileModal = deleteFileModal.find('.modal-close');

  if (closeDeleteFileModal.length !== 0) {
    closeDeleteFileModal.click(hideDeleteFileModal);
  }

  var cancelDeleteFileModal = deleteFileModal.find('.modal-cancel');
...
var deleteFileModal = document.getElementById('deleteFileModal'); // Get all uploaded files and check's if the delete icons is clicked
...
function deleteFile(element) {
  var protocol = String(element.getAttribute('data-protocol'));
  var file = String(element.getAttribute('data-file'));
  var child = element.parentElement.getElementsByClassName('download-file')[0];
  var fileName = child.lastChild.nodeValue.trim();
  deleteFileModal.getElementsByClassName('delete-message')[0].classList.remove('hidden');
  deleteFileModal.getElementsByClassName('modal-message')[0].innerHTML = 'File: <span class="text-indigo-700">' + fileName + '</span>';
  var submitBtn = deleteFileModal.getElementsByClassName('modal-submit')[0];
...
function hideDeleteInitialMessages() {
  var deleteMsg = deleteFileModal.getElementsByClassName('delete-message')[0];
...
deleteFileAjax = function deleteFileAjax(protocol, file, fileName) {
  var deleteFileMsg = deleteFileModal.getElementsByClassName('modal-message')[0];
...
function removeDeletedFile(protocol, file) {
  var element = $(".uploaded-file[data-file='" + file + "'][data-protocol='" + protocol + "']")[0];
