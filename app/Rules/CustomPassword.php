...
class CustomPassword implements Rule, DataAwareRule, ValidatorAwareRule
{
    use Conditionable;
    ...
    public function passes($attribute, $value)
    {
        ...
        if ($this->mixedCase && ! preg_match('/(\p{Ll}+.*\p{Lu})|(\p{Lu}+.*\p{Ll})/u', $value)) {
            $this->fail(trans('validation.custom.mixedCase'));
        }

        if ($this->letters && ! preg_match('/\pL/u', $value)) {
            $this->fail(trans('validation.custom.letters'));
        }

        if ($this->symbols && ! preg_match('/\p{Z}|\p{S}|\p{P}/u', $value)) {
            $this->fail(trans('validation.custom.symbols'));
        }

        if ($this->numbers && ! preg_match('/\pN/u', $value)) {
            $this->fail(trans('validation.custom.numbers'));
        }
        ...
        if ($this->uncompromised && ! Container::getInstance()->make(UncompromisedVerifier::class)->verify([
            ...
            return $this->fail(trans('validation.custom.uncompromised'));
        }
